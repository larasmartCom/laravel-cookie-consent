                        ...
                        <div class="cookies__section">
                            <label for="cookies-policy-check-{{ $category->key() }}" class="cookies__category">
                                ...
                                <span class="cookies__box">
                                    <strong class="cookies__label">{{ __($category->title) }}</strong>
                                </span>
                                @if ($category->description)
                                    <p class="cookies__info">{{ __($category->description) }}</p>
                                @endif
                            </label>
                                ...
                                <ul class="cookies__definitions">
                                    @foreach ($category->getCookies() as $cookie)
                                        <li class="cookies__cookie">
                                            <p
                                                class="inline-block font-medium text-gray-800 text-start dark:text-gray-50">
                                                {{ __($cookie->name) }}</p>
                                            <p class="inline-block text-sm">
                                                {{ \Carbon\CarbonInterval::minutes($cookie->duration)->cascade() }}</p>
                                            @if ($cookie->description)
                                                <p class="text-sm text-gray-500 text-start dark:text-gray-50">
                                                    {{ __($cookie->description) }}</p>
                                            @endif
                                        </li>
                                    @endforeach
                                </ul>
